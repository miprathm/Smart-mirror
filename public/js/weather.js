var weather = {
	...
	token: settings.weather.token,
	
	getWeatherData: function() {
		
		var that = this;
		
		return $.getJSON(this.url+"weather?units=" +
			this.unitType + "&q=" + this.city + "," + this.countryCode + this.token,
			function(data) {
	...
	getForecastData: function() {
		...
		var that = this;
		return $.getJSON(this.url+"forecast/daily?units=" +
			this.unitType + "&cnt=" + (this.forecastDays + 2) + "&q=" +
			this.city + "," + this.countryCode + this.token,
			function(data) {
